...
struct vocab_word {
  long long cn;  //词频
  int *point;    //huffman编码对应内节点的路径, 存放路径上每个非叶结点的索引
...
// start 算法运行的起始时间，会用于计算平均每秒钟处理多少词。 可使用clock()/CLOCKS_PER_SEC来计算时间
clock_t start;
...
int SearchVocab(char *word) {
  ...
  while (1) {
    if (vocab_hash[hash] == -1)                     // 没有这个词 
      return -1;
    if (!strcmp(word, vocab[vocab_hash[hash]].word)) 
      return vocab_hash[hash];                      // 返回单词在词汇表中的索引
...
void SortVocab() {
  ...
  // Sort the vocabulary and keep </s> at the first position   // 保留回车在首位, 对词汇表进行快速排序
  qsort(&vocab[1], vocab_size - 1, sizeof(struct vocab_word), VocabCompare);

  // 词汇重排了，哈希记录的index也乱了，所有的hash记录清除，下面会重建
  ...
  for (a = 0; a < size; a++) {
    ...
    if ((vocab[a].cn < min_count) && (a != 0)) {    // 清除长尾词
      vocab_size--;
      free(vocab[a].word);
    } else {
...
void LearnVocabFromTrainFile() {
  ...
  while (1) {
    ...
    if (vocab_size > vocab_hash_size * 0.7)  // 如果超出装填系数，将词汇表扩容 
      ReduceVocab();
  }

  // 所有词汇加载完毕后进行排序，词频高的靠前
  SortVocab();
    ...
    if (cbow) {  //train the cbow architecture
      ...
      if (cw) {
        ...
        if (hs) for (d = 0; d < vocab[word].codelen; d++) {
          ...
          // 不在expTable内的舍弃掉，邮件问过作者，他说计算精度有限，怕有不好印象，但这里我改成太小的都是0，太大的都是1，运行结果还是有差别的, from web
          if (f <= -MAX_EXP) 
            continue;
          else if (f >= MAX_EXP) 
            continue;
      ...
      for (a = b; a < window * 2 + 1 - b; a++) { 
        ...
        if (a != window) {
          ...
          if (hs) for (d = 0; d < vocab[word].codelen; d++) {
            ...
            // 'g' is the gradient multiplied by the learning rate
            // 这里的code[d]其实是下一层的，code错位了，point和code是错位的！
            g = (1 - vocab[word].code[d] - f) * alpha;
...
void TrainModel() {
  ...
  if (classes == 0) {               // 保存 word vectors
    // Save the word vectors,       // 词汇量，vector维数
